...
You will notice that this section only has the *VSCode* tab enabled — this is because we will not be using Automation Hub to pull collections for this challenge.

[NOTE]
====
A quick note before we dive in: the editor we are using is VSCode and it may be unfamiliar for some folks.  
...
On the *VSCode* tab on the left side you will see that we have a directory opened called `minimal-downstream-from-galaxy` and this directory has a file called `execution-environment.yml`.
...
For a sample of a detailed definition file, check this link:  
