...
# Load DOMAIN and GUID from profile (or environment)
source /etc/profile.d/domain_guid.sh

# --- Prepare directories (run as rhel)
sudo -u rhel mkdir -p /home/rhel/minimal-downstream-with-hub/files/
sudo -u rhel mkdir -p /home/rhel/minimal-downstream-with-hub/solution-definition/

# --- Get token (resolved on host)
...
# --- Write ansible.cfg on host side
...
# --- Write base EE config (host side)
...
# --- Write solution definition EE config (host side)
